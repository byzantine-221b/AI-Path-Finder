import pygame
from path.PygameWidgets import Text
...
Algorithms = ["Breadth-First Search", "Depth-First Search", "A*"]
algo_index = 0
...
class Map():
    ...
    def add_terrain_to_queue(self, location, g, visitor):
        ...
        elif Algorithms[algo_index] in ["A*"]:
            if self.get_terrain_color(location) == WHITE:
    ...
    def get_terrain_last_visitor(self, location):
        return (
        self.terrains[location[1]][location[0]].last_visit.x, self.terrains[location[1]][location[0]].last_visit.y)
...
text1 = Text(text="PATH-FINDING VISUALIZER", align="center", fontsize=22, pos=(WIDTH, 10), dim=(EXTENSION, 0),
             bold=True)
...
text4 = Text(text=">> Relocate start (Red) and end (Blue) points by mouse drag", align="center", fontsize=14,
             pos=(WIDTH, 150), dim=(EXTENSION, 0), wrap=True)
text5 = Text(text=">> Create obstacles by LEFT MOUSE CLICK", align="center", fontsize=14, pos=(WIDTH, 170),
             dim=(EXTENSION, 0), wrap=True)
text6 = Text(text=">> Erase obstacles by RIGHT MOUSE CLICK", align="center", fontsize=14, pos=(WIDTH, 190),
             dim=(EXTENSION, 0), wrap=True)
text7 = Text(text=">> Use Arrow Keys to shift through the algorithms", align="center", fontsize=14, pos=(WIDTH, 210),
             dim=(EXTENSION, 0), wrap=True)
text8 = Text(text=">> Press ENTER key to start Path Finding", align="center", fontsize=14, pos=(WIDTH, 230),
             dim=(EXTENSION, 0), wrap=True)
text9 = Text(text=">> Press R to reset the whole map", align="center", fontsize=14, pos=(WIDTH, 250),
             dim=(EXTENSION, 0), wrap=True)
text10 = Text(text=">> Press C to clear route", align="center", fontsize=14, pos=(WIDTH, 270), dim=(EXTENSION, 0),
              wrap=True)
text11 = Text(text=">> Press P to randomly generate obstacles on the map", align="center", fontsize=14,
              pos=(WIDTH, 290), dim=(EXTENSION, 0), wrap=True)
            ...
            if map.enabled:
                if event.button == 1:
                    if not extension.mouse_is_over(pos):
                        if map.get_index(pos) == map.start:
                            map.on_edit = RED
                        elif map.get_index(pos) == map.end:
                            map.on_edit = BLUE
                        else:
                            map.on_edit = BLACK
                elif event.button == 3:
